...
function run_breakdancer(){
	...
	if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
		...
		$BREAKDANCER/cpp/breakdancer_max -o chr$CHR ${WD}/${ID}.breakdancer.cfg > ${WD}/${ID}.chr${CHR}.SV.output & #xiebo editd chr$CHR
	else
	...
	if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
		...
		do
			...
			do
				...
				$PINDEL/pindel -f $REF -T 2 -i ${WD}/${ID}.pindel.cfg -c chr$c -o ${WD}/${ID}.chr${c} > ${WD}/${ID}.chr${c}.runlog #louhaiyi edited -T 2  #xiebo editd chr$c
			done &
		done
		wait
		for c in $CHRs
		...
		do
			...
			${PINDEL}/pindel -f $REF -i ${WD}/${ID}.chr${CHR}.pindel.cfg -c chr$CHR -o ${WD}/${ID}.chr${CHR} > ${WD}/${ID}.chr${CHR}.runlog #xiebo editd chr$CHR
		done
...
function run_cnvnator(){
	...
	$CNVNATOR -root ${WD}/${ID}.root -unique -tree $BAM > ${WD}/${ID}.cnvnator.runlog #xiebo editd(deleted -genome GRCh37)
	$CNVNATOR -root ${WD}/${ID}.root -his $BIN_SIZE -chrom chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY -d $REFc > ${WD}/${ID}.cnvnator.runlog #xiebo editd -chrom | (deleted -genome GRCh37)
...
function run_breakseq2(){
	...
	do
		...
		$BREAKSEQ2 --nthreads 1 --bams $BAM --work ${WD} --reference $REF --chromosomes chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY --sample $ID --bplib_gff $BS2_bplib --bwa $BWA --samtools $SAMTOOLS --min_span 10 --window $BIN_SIZE --min_overlap 10 2>${WD}/${ID}.breakseq2.runlog #xiebo edited --chromosomes | ----bplib_gff
	done
...
export REFc=/picb/humpopg-bigdata5/xiebo/SV_calling/reference #xiebo edited hg19 -> hg38
#export REF=/picb/humpopg-bigdata5/wangyimin/resource/Human_Genome_hg19/human_g1k_v37.fasta
export REF=/picb/humpopg-bigdata5/xiebo/SV_calling/reference/Homo_sapiens_assembly38.fasta #xiebo edited hg19 -> hg38
#export BS2_bplib=/picb/humpopg-bigdata/furuiqing/Resource/Breakpoints/breakseq2_bplib_20150129.fna
export BS2_bplib=/picb/humpopg-bigdata5/xiebo/SV_calling/reference/bplib.hg38.gff #xiebo edited fna -> gff | hg19 -> hg38
...
export BREAKSEQ2=/picb/humpopg-bigdata5/xiebo/SV_calling/tools/breakseq2/scripts/run_breakseq2.py #xiebo edited breakseq2 version
...
if [ "$run_MetaSV" = 'FALSE' ]; then
	...
	if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
		...
		for c in $CHRs
		do
			MetaSV_arg="$MetaSV_arg_ini --outdir ${WD}/metasv/${ID}/chr${c} --workdir ${WD}/metasv/${ID}/chr${c}/tmp_work --chromosomes chr$c" #xiebo edited chr$c
...
#					$PYTHON $BIN/VCFfilterSize.py $pindelinput $f1_size_l $f1_size_u > ${WD}/pindel/${ID}/${ID}.pindel.chr${c}.runfilter.log
		...
		MetaSV_arg="--reference ${REF} --outdir ${WD}/metasv/${ID}/chr${CHR} --sample ${ID} --num_threads 1 --workdir ${WD}/metasv/${ID}/chr${CHR}/tmp_work --enable_per_tool_output --keep_standard_contigs --chromosomes chr$CHR --mean_read_length $readlength $MS_assembly" #xiebo edited chr$CHR
		if  echo "${MSts[*]}" | grep -w "lumpy" &>/dev/null && [ "${premetasv_flag[1]}" = 'TRUE' ]; then
			lumpyinput=${lumpyvcf:-${WD}/lumpy/${ID}/${ID}.lumpy.vcf}
			if [ "$Raw" = 'FALSE' ]; then
				filterName=".size_${f1_size_l}_${f1_size_u}"
