...
function run_lumpy(){
	...
	wait
	$SAMTOOLS sort ${WD}/${ID}.discordants.unsorted.bam ${WD}/${ID}.discordants &
	$SAMTOOLS sort ${WD}/${ID}.splitters.unsorted.bam ${WD}/${ID}.splitters &
	wait
	...
	if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
		...
		do
			...
			do
				...
				$PINDEL/pindel -f $REF -i ${WD}/${ID}.pindel.cfg -c $c -o ${WD}/${ID}.chr${c} > ${WD}/${ID}.chr${c}.runlog
			done &
		done
		wait
		for c in $CHRs
...
export REFc=/picb/humpopg-geno/louhaiyi/resource/Human_Genome_hg19/GRCh37_fa_files
export REF=/picb/humpopg-ngs/bundle/human_g1k_v37.fasta
...
export BWA=/picb/humpopg-ngs/program/installed/bwa-0.7.10/bwa
export SAMTOOLS=/picb/humpopg-ngs/program/bin/samtools
export PINDEL=/picb/humpopg5/wuzhendong/Pindel
...
export BREAKDANCER=/picb/humpopg2/wuzhendong/program/breakdancer
export LUMPY=/picb/humpopg5/wuzhendong/lumpy-sv
...
GETOPT_ARGS=`getopt -n 'argument error' -o f:i:c:d:s:m:r:aRGFh -al bamfile:,sampleid:,chr:,workdir:,steps:,metasvtools:,dry:,Genotype,Force,pindel_vcf:,breakdancer_naive:,cnvnator_naive:,breakseq2_vcf:,lumpy_vcf:,readlength:,assembly,Raw,help  -- "$@"` ||  eval "usage0; exit 1"
...
do
	if [ "$step" = "all" ]; then
		run=${TOOLs[@]}
		echo "Ok, will run the entire pipeline"
		break
...
#				$PYTHON $BIN/VCFfilterSize.py $pindelinput $f1_size_l $f1_size_u > ${WD}/pindel/${ID}/${ID}.pindel.chr${CHR}.runfilter.log
	...
	if [ "$Raw" = 'FALSE' ] && [ "$MetaSVf" = 'TRUE' ]; then
		...
		if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
			$PYTHON ${BIN}/SeparateFilter_CNVvcf.beta.py ${WD}/metasv/${ID}/${ID}.MetaSV.vcf -${f2_size_l} +${f2_size_u} > ${WD}/metasv/${ID}/${ID}.MetaSV.runfilter.log
		elif [[ "$CHR" =~ ^[1-9]$ || "$CHR" =~ ^1[0-9]$ || "$CHR" =~ ^2[0-2]$ || "$CHR" =~ ^[XY]$ ]]; then
			$PYTHON ${BIN}/SeparateFilter_CNVvcf.beta.py ${WD}/metasv/${ID}/chr${CHR}/variants.vcf.gz -${f2_size_l} +${f2_size_u} > ${WD}/metasv/${ID}/chr${CHR}/MetaSV.runfilter.log
		else
			echo "MetaSV: Unknown Chromosome!"; exit 1
		fi
...
if [ "$geno" = 'TRUE' ] && [ "$MetaSVf" = 'TRUE' ]; then
	...
	if [ "$Raw" = 'FALSE' ]; then
		if [ "$CHR" = 'all' ] || [ "$CHR" = 'autosome' ]; then
			$PYTHON ${BIN}/MetaSV_selfOverlap_v5.py ${WD}/metasv/${ID}/${ID}.MetaSV.DEL.vcf DEL > ${WD}/metasv/${ID}/${ID}.postMetaSV.log
			$PYTHON ${BIN}/MetaSV_selfOverlap_v5.py ${WD}/metasv/${ID}/${ID}.MetaSV.DUP.vcf DUP >> ${WD}/metasv/${ID}/${ID}.postMetaSV.log
			$PYTHON ${BIN}/MetaSV.DEL_DUP.merge.py ${WD}/metasv/${ID}/${ID}.MetaSV.DEL.nonOverlap.vcf ${WD}/metasv/${ID}/${ID}.MetaSV.DUP.nonOverlap.vcf >> ${WD}/metasv/${ID}/${ID}.postMetaSV.log
			$PERL ${BIN}/Convert_VCF2CNVR.pl ${WD}/metasv/${ID}/${ID}.MetaSV.DEL_DUP.nonOverlap.vcf >> ${WD}/metasv/${ID}/${ID}.postMetaSV.log
			echo "exit" |cat ${WD}/metasv/${ID}/${ID}.MetaSV.DEL_DUP.nonOverlap.region.txt - | $CNVNATOR -root ${WD}/cnvnator/${ID}/${ID}.root -genotype $BIN_SIZE 1>${WD}/metasv/${ID}/${ID}.MetaSV.DEL_DUP.genotype.txt 2>${WD}/metasv/${ID}/${ID}.postMetaSV.err
		elif [[ "$CHR" =~ ^[1-9]$ || "$CHR" =~ ^1[0-9]$ || "$CHR" =~ ^2[0-2]$ || "$CHR" =~ ^[XY]$ ]]; then
			$PYTHON ${BIN}/MetaSV_selfOverlap_v5.py ${WD}/metasv/${ID}/chr${CHR}/variants.DEL.vcf DEL > ${WD}/metasv/${ID}/chr${CHR}/postMetaSV.log
			$PYTHON ${BIN}/MetaSV_selfOverlap_v5.py ${WD}/metasv/${ID}/chr${CHR}/variants.DUP.vcf DUP >> ${WD}/meatsv/${ID}/chr${CHR}/postMetaSV.log
			$PYTHON ${BIN}/MetaSV.DEL_DUP.merge.py ${WD}/metasv/${ID}/chr${CHR}/variants.DEL.nonOverlap.vcf ${WD}/metasv/${ID}/chr${CHR}/variants.DUP.nonOverlap.vcf >> ${WD}/metasv/${ID}/chr${CHR}/postMetaSV.log
			$PERL ${BIN}/Convert_VCF2CNVR.pl ${WD}/metasv/${ID}/chr${CHR}/variants.DEL_DUP.nonOverlap.vcf >> ${WD}/metasv/${ID}/chr${CHR}/postMetaSV.log
			echo "exit" |cat ${WD}/metasv/${ID}/chr${CHR}/variants.DEL_DUP.nonOverlap.region.txt - | $CNVNATOR -root ${WD}/cnvnator/${ID}/${ID}.root -genotype $BIN_SIZE 1>${WD}/metasv/${ID}/chr${CHR}/variants.DEL_DUP.genotype.txt 2>${WD}/metasv/${ID}/chr${CHR}/postMetaSV.err
		else
